  ...
  private:
    ...
    SNMALLOC_SLOW_PATH capptr::Alloc<void> alloc_not_small(size_t size)
    {
      ...
      return check_init([&](CoreAlloc* core_alloc) {
        ...
        return capptr_chunk_is_alloc(capptr_to_user_address_control(
          capptr_tint_region(chunk, large_size_to_chunk_size(size), 15))); // xxx hardcoded tint
      });
    }
...
#  ifdef __CHERI_PURE_CAPABILITY__
       ...
       */
      /* XXX may not be required with csetbounds check */
...
#  endif
        if (SNMALLOC_LIKELY(CoreAlloc::dealloc_local_object_fast(
              entry, rederived_ptr, local_cache.entropy)))
          return;
...
#  endif
        ...
        {
          local_cache.remote_dealloc_cache.template dealloc<sizeof(CoreAlloc)>(
            remote->trunc_id(), rederived_ptr, key_global);
#  ifdef SNMALLOC_TRACING
          message<1024>(
...
#  endif
        ...
        dealloc_remote_slow(rederived_ptr);
        return;
      }
