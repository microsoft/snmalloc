...
#endif
// WeakAVL tree needs some libraries with trace enabled.
#include "snmalloc/snmalloc.h"

struct NodeRef
{
  // The weakavl tree is going to be used inside the pagemap,
  // and the weakavl tree cannot use all the bits.  Applying an offset
...
// Bottom bit of left is used to store the rank parity.
// We shift the fields up to make room for the parity bit.
struct node
{
  size_t left;
  size_t right;
...
public:
  ...
  static void set_tree_tag(key k, uint8_t new_tree_tag)
  {
    bool new_tag = (new_tree_tag & 1) != 0;
    if (new_tag != tree_tag(k))
      array[k].left ^= 1;
  }
...
void test(size_t size, unsigned int seed)
{
  ...
  snmalloc::WeakAVLTree<Rep, true, TRACE> tree;
