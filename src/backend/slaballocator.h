...
namespace snmalloc
{
  ...
  public:
...
#ifdef SNMALLOC_TRACING
        std::cout << "Reuse slab:" << slab.unsafe_capptr << " slab_sizeclass " << slab_sizeclass << " size " << slab_size << std::endl;
#endif
        MetaEntry entry{meta, remote};
        BackendAllocator::set_meta_data(h, address_cast(slab), slab_size, entry);
        return {slab, meta};
      }
...
#ifdef SNMALLOC_TRACING
      std::cout << "Create slab:" << slab.unsafe_capptr << " slab_sizeclass " << slab_sizeclass << " size " << slab_size << std::endl;
#endif
      return {slab, meta};
    }
    ...
    {
#ifdef SNMALLOC_TRACING
      std::cout << "Return slab:" << p->slab.unsafe_capptr 
